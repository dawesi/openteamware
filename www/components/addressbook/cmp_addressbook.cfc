...
<cfcomponent output='false'>
	...
	<cffunction access="public" name="RemoteEditDataAvailable" output="false" returntype="boolean"
			hint="are remote edit data available for a certain contact?">
		<cfargument name="entrykey" type="string" required="true"
			hint=" addressbook key">
		
		<cfset var q_select_re_data_available = 0 />
		<cfinclude template="queries/q_select_re_data_available.cfm">

		<cfreturn (q_select_re_data_available.count_id IS 1)>
	</cffunction>
	
	<!--- return the corresponding entrykeys --->
	<cffunction access="public" name="RemoteEditDateAvailableForUser" output="false" returntype="string">
		<cfargument name="userkey" type="string" required="true">
		
		<cfinclude template="queries/q_select_re_data_available_for_user.cfm">
		
		<cfreturn ValueList(q_select_re_data_available_for_user.entrykey)>
	</cffunction>
	
	<cffunction access="public" name="GetRemoteEditJobdata" output="false" returntype="query">
		<cfargument name="entrykey" type="string" required="true"
			hint="remote edit job key">
		
		<cfreturn application.components.cmp_dao_addressbook.listByProperty('remoteedit.RemoteEditJobs', 'entrykey', arguments.entrykey) />
		
	</cffunction>
	
	<cffunction access="public" name="GetRemoteEditJobDetails" output="false" returntype="query"
			hint="get the remote edit job data using the address book key">
		<cfargument name="entrykey" type="string" required="yes">
	...
	<cffunction access="public" name="RemoteEditJobAvailable" output="false" returntype="boolean">
		<cfargument name="entrykey" type="string" required="true">
		
		<cfinclude template="queries/q_select_re_job_available.cfm">
		
		<cfreturn (q_select_re_job_available.count_id IS 1)>
	</cffunction>
	
	<cffunction access="public" name="DeleteRemoteEditJob" output="false" returntype="boolean">
		<cfargument name="entrykey" type="string" required="true"
			hint="adressbook entrykey (contact, ...)">
		<cfargument name="deleteremoteeditdata" type="boolean" default="true" required="false">
		
		<cfset var a_orm = 0 />
		
		<cfinclude template="queries/q_delete_re_edit_job.cfm">
	
		<cfif arguments.deleteremoteeditdata>
			
			<cfscript>
				a_orm = application.components.cmp_dao_addressbook.get("remoteedit.RemoteEditData", arguments.entrykey);
			</cfscript>
			
			<!--- <cfmail from="hp@openTeamware.com" to="hp@openTeamware.com" subject="orm" type="html">
			<cfdump var="#a_orm#">
			</cfmail> --->
	...
	<cffunction access="public" name="GetRemoteEditData" output="false" returntype="query">
		<!--- entrykey of contact --->
		<cfargument name="entrykey" type="string" required="true">
		
		<cfinclude template="queries/q_select_re_data.cfm">
		
		<cfreturn q_select_re_data>		
	</cffunction>
	
	<cffunction access="public" name="ActivateRemoteEdit" output="false" returntype="struct"
			hint="send remote edit email">
		<cfargument name="entrykey" type="string" required="true"
			hint="entrykey of the contact">
		<cfargument name="securitycontext" type="struct" required="true">
		<cfargument name="usersettings" type="struct" required="true">
		<cfargument name="forcenewsend" type="boolean" required="no" default="false"
			hint="force a second try even if the mail has already been sent">
		<cfargument name="ownintromsg" type="string" required="false" default=""
			hint="if given, use this intro message instead of the default one ...">
		
		<cfset var stReturn = GenerateReturnStruct() />
		<cfset var a_str_job_key = CreateUUID() />
		<cfset var a_int_lang_of_contact = 0 />
		<cfset var q_select_user_data = application.components.cmp_user.GetUserdata(userkey = arguments.securitycontext.myuserkey) />
		<cfset var a_int_current_user_language = client.langno />
		<cfset var q_select_contact = 0 />
		<cfset var iLangNo = 0 />
		<cfset var stReturn_crm_history = 0 />
		<cfset var q_insert_re_job = 0 />
		<cfset var q_select_contact_re = 0 />
		<cfset var a_str_from_adr = '' />
		<cfset var a_str_mail_text = '' />
		<cfset var a_str_mail_subject = '' />
		<cfset var a_str_page_include_remote_edit_text_version = '' />
		<cfset var a_str_page_include_ad_remote_edit_bottom = '' />
		<cfset var a_str_addressbook_vcard_entrykey = '' />
		<cfset var a_bol_user_vcard_available = false />
		<cfset var sVcard_filename = '' />
		<cfset var a_struct_load_contact_vcard_attach = 0 />
		<cfset var sVcard = '' />
		<cfset var sVcard_directory = '' />
		<cfset var a_str_salutation = '' />
		<cfset var a_str_anrede = '' />
		...
		<cfset var a_struct_big_logo = application.components.cmp_customize.GetCustomStyleData(usersettings = arguments.usersettings, entryname = 'big_logo') />
		...
		<!--- check for special includes now ... --->
		...
		<cfif StructKeyExists(a_struct_load_contact, 'q_select_contact') AND a_struct_load_contact.rights.edit>
		
			<cfif Len(ExtractEmailadr(a_struct_load_contact.q_select_contact.email_prim)) IS 0>
				<!--- no primary email address provided ... --->
				<cfreturn SetReturnStructErrorCode(stReturn, 1105) />
			</cfif>
			
			<!--- if language is given and supported, use this, otherwise default language ... --->
			<cfif (Len(a_struct_load_contact.q_select_contact.lang) IS 2) AND
				  (IsLanguageSupportedBySystem(a_struct_load_contact.q_select_contact.lang))>
				  	<!--- language has been found and is supported --->
					<cfset a_int_lang_of_contact = application.components.cmp_lang.GetLanguageNumberByShortName(a_struct_load_contact.q_select_contact.lang) />
					<cfset iLangNo = application.components.cmp_lang.GetLanguageNumberByShortName(a_struct_load_contact.q_select_contact.lang) />
			<cfelse>
					<cfset a_int_lang_of_contact = q_select_user_data.defaultlanguage />
					<cfset iLangNo = q_select_user_data.defaultlanguage />
			</cfif>

			<!--- activate --->
			<cfinclude template="queries/q_insert_re_job.cfm">
			
			<!--- send email --->
			<cfif a_bol_new OR arguments.forcenewsend>
				<cfinclude template="utils/inc_send_re_data.cfm">
			</cfif>
			
			<!--- if the language has change, make the change undone --->
			<cfif a_int_current_user_language NEQ client.langno>
				<cfset client.langno = a_int_current_user_language />
			</cfif>
		
			<cfset stReturn.jobentrykey = a_str_job_key />
			...
			<cfinvoke component="#application.components.cmp_crmsales#" method="CreateHistoryItem" returnvariable="stReturn_crm_history">
				...
				<cfinvokeargument name="comment" value="Remote Edit Anforderung verschickt">
				<cfinvokeargument name="dt_created" value="#GetODBCUTCNow()#">
				<cfinvokeargument name="linked_objectkey" value="#a_str_job_key#">
				<cfinvokeargument name="categories" value="remoteedit">
			</cfinvoke>
	...
	<cffunction access="private" name="WriteUnknownReField" output="false" returntype="string"
			hint="return a unknown string in case the field is empty">
		<cfargument name="s" type="string" required="true" hint="string to check">
		<cfargument name="langno" type="numeric" default="0" required="true"
			hint="id of lang to return 'unknown' for if no data is found ...">
		
		<cfif Len(s) IS 0>
			<cfreturn '<font color="red">' & htmleditformat(application.components.cmp_lang.GetLangValExt(langno = arguments.langno, entryid='cm_wd_empty')) & '</font>' />
		<cfelse>
	...
	<cffunction access="public" name="SaveRemoteEditData" output="false" returntype="boolean">
		<!--- entrykey ... --->
		<cfargument name="entrykey" type="string" required="true">		
		<cfargument name="firstname" type="string" required="false" default="">
		<cfargument name="surname" type="string" required="false" default="">
		...
		<cfargument name="p_mobile" type="string" required="false" default="">
		<cfargument name="p_url" type="string" required="false" default="">	
		<cfargument name="skypeusername" type="string" required="no" default="">			
		
		<cfargument name="notice" type="string" required="false" default="">
		
		<cfinclude template="queries/q_insert_remote_edit_data.cfm">
		
		<!--- update that remote edit data is available --->
