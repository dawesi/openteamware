...
<cfcomponent output=false hint="Calendar component">
	...
	<cffunction access="public" name="TransferOwnership" output="false" returntype="boolean">
		...
		<cfinvoke component="cmp_task" method="UpdateEvent" returnvariable="a_bol_return">
			<cfinvokeargument name="entrykey" value="#arguments.entrykey#">
			<cfinvokeargument name="securitycontext" value="#arguments.securitycontext#">
	...
	<!--- ************* virtual calendar functions *************** --->

	<cffunction access="public" name="GetVirtualCalendarsOfUser" returntype="query" output="false">
		<!--- the userkey ... --->
		<cfargument name="userkey" type="string" required="true">
		
        <cfset var q_select_virtual_calendars = 0 />

		<cfinclude template="queries/q_select_virtual_calendars.cfm">
		<cfreturn q_select_virtual_calendars>
	</cffunction>
	
	
	<cffunction access="public" name="GetPublicVirtualCalendars" returntype="query" output="false"
				hint="return available public calendars">
				
        <cfset var q_select_public_virtual_calendars = 0 />
		<cfinclude template="queries/q_select_public_virtual_calendars.cfm">
		<cfreturn q_select_public_virtual_calendars>
	</cffunction>
	
	<cffunction access="public" name="GetMyOrSubscribedCalendars" returntype="query" output="false"
				hint="Return calendars of the user and calendars the user is subscribed into">
		<cfargument name="userkey" type="string" required="true">

        <cfset var q_select_my_or_subscribed_virtual_calendars = 0 />

		<cfinclude template="queries/q_select_my_or_subscribed_virtual_calendars.cfm">
		<cfreturn q_select_my_or_subscribed_virtual_calendars>
	</cffunction>
	
	<cffunction access="public" name="GetVirtualCalendarSubscriptions" returntype="string" output="false"
				hint="Return the subscriptions to virtual calendars of an user, return just the entrykeys">
		<cfargument name="userkey" type="string" required="true">

        <cfset var q_select_virtual_calendars_subscriptions = 0 />

		<cfinclude template="queries/q_select_virtual_calendars_subscriptions.cfm">
		
		<cfreturn ValueList(q_select_virtual_calendars_subscriptions.virtualcalendarkey) />
	</cffunction>
	
	
	<cffunction access="public" name="GetVirtualCalendarSubscriptionsAsQuery" returntype="query" output="false">
		<cfargument name="userkey" type="string" required="true"
		...
		<cfinclude template="queries/q_select_virtual_calendars_subscriptions.cfm">
		<cfreturn q_select_virtual_calendars_subscriptions />
	</cffunction>


	<cffunction access="public" name="GetVirtualCalendarOfUser" returntype="struct" output="false"
			hint="return calendar (of the user) for provided entrykey">
		<cfargument name="userkey" type="string" required="true">
		<cfargument name="virtualcalendarkey" type="string" required="true"
			hint="entrykey of virtual calendar">

		<cfset var stReturn = GenerateReturnStruct()>
        <cfset var q_select_virtual_calendar = 0 />
        
		<cfinclude template="queries/q_select_virtual_calendar.cfm">
		
		<cfif q_select_virtual_calendar.recordcount EQ 0>
			<cfreturn SetReturnStructErrorCode(stReturn, 13001)>
		</cfif>

		<cfset stReturn.q_select_virtual_calendar = q_select_virtual_calendar>
		<cfreturn SetReturnStructSuccessCode(stReturn)>
	</cffunction>


	<cffunction access="public" name="CreateUpdateVirtualCalendar" returntype="struct" output="false">
		<cfargument name="securitycontext" type="struct" required="true">
		<cfargument name="usersettings" type="struct" required="true">
		...
		<cfset var stReturn = GenerateReturnStruct()>
        <cfset var q_insert_virtual_calendar = 0 />
        <cfset var q_update_virtual_calendar = 0 />
		
		<cfset var a_bool_exists_calendar = VirtualCalendarWithThisNameAlreadyExists(
											userkey = arguments.securitycontext.myuserkey,
											title = arguments.database_values.title,
											virtualcalendarkey = arguments.database_values.entrykey)>
		<cfif a_bool_exists_calendar>
			<cfreturn SetReturnStructErrorCode(stReturn, 13002)>
		</cfif>

		<cfif action_type IS 'Create'>
			<cfinclude template="queries/q_insert_virtual_calendar.cfm">
		<cfelse>
			<cfinclude template="queries/q_update_virtual_calendar.cfm">
		</cfif>
		<cfreturn SetReturnStructSuccessCode(stReturn)>
	</cffunction>


	<cffunction access="public" name="VirtualCalendarWithThisNameAlreadyExists" returntype="boolean" output="false">
		<cfargument name="userkey" type="string" required="true">
		<cfargument name="title" type="string" required="true">
		<cfargument name="virtualcalendarkey" type="string" required="false" hint="entrykey of the calendar that will be excluded from check">

        <cfset var q_count_virtual_calendars = 0 />

		<cfinclude template="queries/q_count_virtual_calendars.cfm">
		<cfreturn (q_count_virtual_calendars.count_id GT 0)>
	</cffunction>
	
	
	<cffunction access="public" name="DeleteVirtualCalendar" returntype="struct" output="false"
					hint="delete a definition of an own virtual calendar">
		<cfargument name="securitycontext" type="struct" required="true">
		<cfargument name="usersettings" type="struct" required="true">				
		<cfargument name="virtualcalendarkey" type="string" required="true" hint="entrykey of virtual calendar">
		<cfargument name="dodeleteappointmentsaswell" type="boolean" required="true" 
					hint="specifies if the events of this calendar should be deleted or moved to default calendar">
			
		<cfset var stReturn = GenerateReturnStruct()>
        <cfset var q_delete_events_from_calendar = 0 />
        <cfset var q_update_virtualcalendar_deleted_item = 0 />
        <cfset var q_remove_subscriptions_from_calendar = 0 />
        <cfset var q_delete_virtual_calendar = 0 />
		
		<cfif arguments.dodeleteappointmentsaswell>
			<!--- delete all events of this virtual calendar --->
			<cfinclude template="queries/q_delete_events_from_calendar.cfm">
		<cfelse>
			<!--- move all the events from virtual calendar (we are going to delete) to default calendar --->
			<cfinclude template="queries/q_update_virtualcalendar_deleted_item.cfm">
		</cfif>
		
		<!--- remove subscriptions for calendar we are going to delete (if any) --->
		<cfinclude template="queries/q_remove_subscriptions_from_calendar.cfm">
		
		<!--- include sql DELETE command --->
		<cfinclude template="queries/q_delete_virtual_calendar.cfm">
		
		<cfreturn SetReturnStructSuccessCode(stReturn)>
	</cffunction>	

	
	<cffunction access="public" name="SubscribeToPublicVirtualCalendar" returntype="struct" output="false"
					hint="subscribe to a given virtual calendar">
		<cfargument name="securitycontext" type="struct" required="true">
		<cfargument name="usersettings" type="struct" required="true">
		<cfargument name="virtualcalendarkey" type="string" required="true"	hint="entrykey of public virtual calendar">	
		
		<cfset var stReturn = GenerateReturnStruct() />
        <cfset var q_insert_subscribe_public_virtual_calendar = 0 />
		
		<cfinclude template="queries/q_insert_subscribe_public_virtual_calendar.cfm">
		
		<cfreturn SetReturnStructSuccessCode(stReturn) />
	</cffunction>

	
	<cffunction access="public" name="UnsubscribeFromPublicVirtualCalendar" returntype="struct" output="false"
			hint="unsubscribe to a given virtual calendar">
		<cfargument name="securitycontext" type="struct" required="true">
		<cfargument name="usersettings" type="struct" required="true">	
		<cfargument name="subscriptionkey" type="string" required="true"
			hint="entrykey of subscription">
		
		<cfset var stReturn = GenerateReturnStruct()>
        <cfset var q_remove_subscription = 0 />
		
		<cfinclude template="queries/q_remove_subscription.cfm">
