		...
		<cfelse>
			...
			<cfswitch expression="#a_struct_form_element.datatype#">
				...
				<cfcase value="date,datetime">
					...
					<cfswitch expression="#a_str_device_type#">
						<cfcase value="pda">
							<!--- pda --->
							<cfif IsDate(a_str_date_value)>
								<cfset a_int_dt_pda_day = Day(a_str_date_value) />
								<cfset a_int_dt_pda_month = Month(a_str_date_value) />
								<cfset a_int_dt_pda_year = Year(a_str_date_value) />
							<cfelse>
								<cfset a_int_dt_pda_day = Day(Now()) />
								<cfset a_int_dt_pda_month = Month(Now()) />
								<cfset a_int_dt_pda_year = Year(Now()) />
							</cfif>
							
							<select name="#a_struct_form_element.input_name#_date_day">
								<cfloop from="1" to="31" index="ii">
									<option value="#ii#" #WriteSelectedElement(a_int_dt_pda_day, ii)#>#ii#</option>
								</cfloop>
							</select>
							.
							<select name="#a_struct_form_element.input_name#_date_month">
								<cfloop from="1" to="12" index="ii">
									<option value="#ii#" #WriteSelectedElement(a_int_dt_pda_month, ii)#>#ii#</option>
								</cfloop>
							</select>
							.
							<select name="#a_struct_form_element.input_name#_date_year">
								<cfloop from="2006" to="2012" index="ii">
									<option value="#ii#" #WriteSelectedElement(a_int_dt_pda_year, ii)#>#ii#</option>
								</cfloop>
							</select>
						</cfcase>
						<cfdefaultcase>
							...
							<cfset tmp = AddJSToExecuteAfterPageLoad('DoAddCal#a_str_add_cal_name#()', a_str_js_to_add) />
