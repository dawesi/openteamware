...
<cfcomponent output='false'>
	...
	<cffunction access="public" name="CheckLoginData" output="false" returntype="struct"
		...
		<cfif arguments.check_restrictions>
			...
			<!--- trial phase? ... --->
			<cfif application.components.cmp_licence.TrialPhaseExipired(companykey = stReturn.q_userdata.companykey)>
				<cfreturn SetReturnStructErrorCode(stReturn, 200) />
			</cfif>
			
			<!--- open bills? ... --->
			<cfif (application.components.cmp_licence.CheckDunningLetters(companykey = stReturn.q_userdata.companykey, dunningminlevel = 2).recordcount GT 0)>
				<cfreturn SetReturnStructErrorCode(stReturn, 201) />
			</cfif>
