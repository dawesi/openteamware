...
<cfcomponent output="false">
	...
	<cffunction access="public" name="InsertUpdateRecord" output="false" returntype="struct"
		...
		<cfset arguments.database = ReturnEvalDatabaseName(arguments.database) />
	...
	<cffunction access="private" name="ReturnEvalDatabaseName" output="false" returntype="string"
			hint="return the correct database name (evaluate)">
		<cfargument name="database" type="string" required="true">
		
		<cfif FindNoCase('##', arguments.database) IS 0>
			<cfreturn arguments.database />
		<cfelse>
			<cfreturn Evaluate(arguments.database) />
		</cfif>
