<!--- //
	...
	Module:		Storage
	Description:Add file from storage to email
	

	
	
	display all folders and all files
...
<!--- <cfinvoke component="#application.components.cmp_storage#"
	method="SimpleGetFileListOfDirectories"
	returnvariable="a_struct_all_files"
	directorykeys="#ValueList(q_select_directories.entrykey)#"
	securitycontext="#request.stSecurityContext#"
	usersettings="#request.stUserSettings#">
</cfinvoke>

<cfset q_select_all_files = a_struct_all_files.q_select_files /> --->
		...
		<cfif a_current_hint.counter lte 0>
			...
			<div style="padding-left:#a_int_padding_left#px;padding-top:3px; ">
				...
				<cfloop query="q_select_files">
					<input type="checkbox" name="frmFilesentrykeys" value="#q_select_files.entrykey#" class="noborder"> <a target="_blank" href="../storage/default.cfm?action=ShowFile&entrykey=#q_select_files.entrykey#">#htmleditformat(q_select_files.name)#</a> (#ByteConvert(q_select_files.filesize)#)<br>
				</cfloop>
...
<cffunction access="private" name="LoopOutput" returntype="void" output="true">
	...
	<cfoutput query="q_select_current_level">
		<li>
			...
			<!--- select from the whole list the right files --->
			<!--- <cfquery name="q_select_files" dbtype="query">
			SELECT
				filesize,name,entrykey,contenttype
			FROM
				q_select_all_files
			WHERE
				parentdirectorykey = <cfqueryparam cfsqltype="cf_sql_varchar" value="#q_select_current_level.entrykey#">
			;
			</cfquery>
			...
			<ul>
				<cfloop query="q_select_files">
				<li>
					<input type="checkbox" class="noborder" value="#htmleditformat(q_select_files.entrykey)#" name="frmFilesentrykeys" />
					#application.components.cmp_tools.GetImagePathForContentType(q_select_files.contenttype)# #htmleditformat(q_select_files.name)#
				</li>
				</cfloop>
			</ul>
			</cfif> --->
...
<script type="text/javascript">
	...
	function LoadAddAttFolderContent(sender,entrykey) {
		...
		$.get("/storage/",
			  { directorykey: entrykey, action: 'SimpleSelectAttLoadFilesOfDirectory' },
			  function(data){
